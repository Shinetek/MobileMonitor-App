...
(function () {
    ...
    angular.module("starter.controllers")
        ...
        .controller("MCSCapabilityController", MCSCapabilityCtrlFn)
        .controller("CVSCapabilityController", CVSCapabilityCtrlFn);

    NRSCapabilityCtrlFn.$inject = ["CapabilityServices", "$scope"];
    MCSCapabilityCtrlFn.$inject = ["CapabilityServices", "$scope"];
    CVSCapabilityCtrlFn.$inject = ["CapabilityServices", "$scope"];

    DTSMRSCapabilityCtrlFn.$inject = ["CapabilityServices", "$scope", "$stateParams"];



    function CVSCapabilityCtrlFn(CapabilityServices, $scope) {
        var self = this;
        ...
        function _getCapability(next) {
            CapabilityServices.getCapability("CVS", function (doc) {
                if (!doc) return next();
    ...
    function MCSCapabilityCtrlFn(CapabilityServices, $scope) {
        var self = this;

        // 当前navTab
        self.instNavCurrentItem = 'TELECM';
        ...
        function _getCapability(next) {
            CapabilityServices.getCapability("mcs", function (doc) {
                if (!doc) return next();
                var inst = self.instNavCurrentItem.toUpperCase();
                var newdata = []
                self.capabilityItems.splice(0, self.capabilityItems.length);
                for(var i = 0; i < doc.data.length; i++){
                    if(doc.data[i].param.length != 0){
                        newdata.push(doc.data[i])
                    }
                }
                for (var i = 0; i < newdata.length; i++) {
                    if (newdata[i]["instrument"] === inst) {
                        newdata[i]["param"].forEach(function (item) {
                            if (item.status === "success") {
                                item.statusImg = "img/check_128px_1180491_easyicon.net.png";
                            } else {
                                item.statusImg = "img/error_128px_1181827_easyicon.net.png";
